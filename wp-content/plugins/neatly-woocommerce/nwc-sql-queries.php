...
FROM {$wpdb->prefix}posts p WHERE p.post_type='shop_order' AND p.post_status IN('wc-completed', 'wc-processing', 'wc-on-hold', 'wc-refunded') AND DATE(p.post_date) BETWEEN %s AND %s GROUP BY DATE(p.post_date)");
...
FROM {$wpdb->prefix}posts p WHERE p.post_type='shop_order' AND p.post_status IN('wc-completed', 'wc-processing', 'wc-on-hold', 'wc-refunded') AND DATE(p.post_date) BETWEEN %s AND %s");

define("NWC_API_ORDERSTATS_SQL1_REFUNDED", "SELECT
    COUNT(*) AS order_count,
    SUM((SELECT meta_value FROM {$wpdb->prefix}postmeta pm1 WHERE pm1.post_id = p.ID AND pm1.meta_key='_order_total')) AS order_total,
    SUM((SELECT meta_value FROM {$wpdb->prefix}postmeta pm2 WHERE pm2.post_id = p.ID AND pm2.meta_key='_refund_amount')) AS refunded_total
FROM {$wpdb->prefix}posts p WHERE p.post_type='shop_order_refund' AND p.post_status = 'wc-completed' AND DATE(p.post_date) BETWEEN %s AND %s");
...
WHERE woi.order_item_type='line_item' AND p1.post_status = 'wc-completed' AND DATE(p1.post_date) BETWEEN %s AND %s");
...
FROM (
    ...
    FROM (
        ...
        FROM {$wpdb->prefix}posts p LEFT JOIN
        (SELECT post_id, meta_value FROM {$wpdb->prefix}postmeta pm WHERE pm.meta_key='_billing_email') pm1 ON pm1.post_id = p.ID
        WHERE p.post_type='shop_order' AND DATE(p.post_date) BETWEEN %s AND %s
