<tr class="css-customization-element css-item-{{identifier}}-{{variable-name}}">
	...
	<td class="popupally-customization-table-content">
		{{start-inherit-code}}
		<input class="full-width" id="image-input-{{preview-element}}-{{variable-name}}" {{auto-adjust}} name="{{base-setting-name}}[css][{{variable-name}}]" type="text" value="{{value}}" preview-update-target-css-background-img-hover="#popupally-preview-{{preview-element}}" />
