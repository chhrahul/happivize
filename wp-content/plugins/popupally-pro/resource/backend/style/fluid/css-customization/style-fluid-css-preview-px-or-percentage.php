<tr class="css-customization-element css-item-{{identifier}}-{{variable-name}}">
	...
	<td class="popupally-customization-table-content">
		{{start-inherit-code}}
		<div><input size="8" name="{{base-setting-name}}[css][{{variable-name}}]" {{auto-adjust}} verify-px-pct-input="#px-pct-input-error-{{preview-element}}-{{variable-name}}" type="text" value="{{value}}" preview-update-target-css="#popupally-preview-{{preview-element}}" preview-update-target-css-property="{{variable-name}}"> Please enter 'px' or '%'</div>
