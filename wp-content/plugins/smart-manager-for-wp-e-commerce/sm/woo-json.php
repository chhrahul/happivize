                ...
                if ($show_variation === true) {
                    ...
                    if ( $records[$i]['post_parent'] != 0 && $product_type_parent[0] != "grouped" ) {
                        ...
                        if($_POST['SM_IS_WOO16'] == "true") {
                            ...
                            //$records[$i]['speakers_bio_name'] = 'Hello 1';
                        }
                        $variation_names = '';
                    ...
                    // } else if ($records[$i]['post_parent'] == 0 && $product_type[0] == 'variable') {
                       ...
                       // $records[$i]['speakers_bio_name'] = 'Hello 2';
                        if ( empty( $records[$i]['_regular_price'] ) && $records[$i]['_regular_price'] != 0 ) {
                           ...
                           // $records[$i]['speakers_bio_name'] = 'Hello 3';
                        }
                    }
                    ...
                    if ($product_type == 'variable') {
                        ...
                        if ($_POST['SM_IS_WOO21'] == "true" || $_POST['SM_IS_WOO22'] == "true") {
                           ...
                           //$records[$i]['speakers_bio_name'] = get_post_meta( $variation_reg_price[$records[$i]['id']], 'speakers_bio_name', true ); 
                            ...
                            //$records[$i]['speakers_bio_name'] = 'Hello 4';
                           ...
                           // $records[$i]['speakers_bio_name'] = 'Hello 5';
                            $records[$i]['_sale_price'] = $records[$i]['_min_variation_sale_price'];    
                        }
                        
                    } else {
                        $records[$i]['_regular_price'] = (isset($records[$i]['_regular_price'])) ? trim( $records[$i]['_regular_price'] ) : '';
                       // $records[$i]['speakers_bio_name'] = 'Hello 6';
                    ...
                    $records[$i]['_sku'] = $records[$i]['speakers_bio_name'];
                    
                     //global $wpdb;
                    $mylink = $wpdb->get_row( "SELECT post_modified FROM hp_posts WHERE ID = ".$records[$i]['id'] );
                    $records[$i]['category'] = $mylink->post_modified;
                    
                    
                    $records[$i]['_sale_price'] = $records[$i]['_sale_price'];
                }
                 //$records[$i]['speakers_bio_name'] = 'Hello'; 
                unset($records[$i]['prod_othermeta_value']);
                unset($records[$i]['prod_othermeta_key']);
    ...
    //                                                                                          
                ...
                foreach ( $results as $data ) {
                    ...
                    if(count($meta_key) == count($meta_value)){
                                                        ...
                                                        if (!empty($order_items[$data['id']])) {
                                                            ...
                                                            foreach ( $order_items[$data['id']] as $order_item) {
                                                                ...
                                                                // if ($data['id'] == $order_item['order_id']) {
                                                                   ...
                                                                   /*  $order_d = new WC_Order( $order_item['order_id'] );
                                                                      
                                                                      $items = $order_d->get_items();    */
                                //global $wpdb;
                              $result_n = $wpdb->get_results('select * FROM hp_woocommerce_order_items where order_id='.$order_item['order_id']);

                                
                                if ( count( $results ) > 0 ) {
                                $product_f_name = array();
                                foreach ( $result_n as $item_n ) {
                                    $product_f_name[] = $item_n->order_item_name;                                                                
                                }
                                 $product_f_name = implode(", ",$product_f_name);
                                }
                               
                              /*  $product_f_name = rtrim($product_f_name,", "); 
                                 */
                        ...
                        $data['details'] = !empty($data['details']) ? ( ($data['details'] == 1) ? $product_f_name . '' : $product_f_name . '' ) : ''; 
                        $data['products_name'] = substr($data['products_name'], 0, -2); //To remove extra comma ', ' from returned string                                                                              
                    }
...
if (isset ( $_GET ['func_nm'] ) && $_GET ['func_nm'] == 'exportCsvWoo') {
    ...
    switch ( $active_module ) {
        ...
        case 'Products':
                ...
                $columns_header['_speakers_bio_name']                  = __('Speakers Bio Name', $sm_text_domain);
        ...
        // meta_key required for new products, that are entered through Smart Manager   
            // if (count($post_meta_info) <= 0 || count($post_meta_info) < 23) {
                $post_meta_reqd_keys = array(
                            ...
                            '_sale_price', 'speakers_bio_name', '_length', '_width' ,
                            ...
                            '_stock', '_manage_stock', '_backorders','speakers_bio_name');
...
//                      '_price'                    => ( ($obj->post_parent == 0 && $obj->product_type != 'variable') || ($product_type_parent[0] == "grouped") ) ? $price : $obj->_regular_price,
                        ...
                        '_speakers_bio_name'                   => isset($obj->_speakers_bio_name) ? $obj->_speakers_bio_name : '',
