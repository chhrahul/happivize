...
//		var_dump( $this->items );
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			case 'active':
				return "<input type='checkbox' data-action='change-schedule-status' data-id='{$item['id']}' " . ( ! isset( $item['active'] ) || $item['active'] ? 'checked' : '' ) . "/> " . $item[ 'id' ];
				break;
			case 'title':
			...
			case 'last_run':
				...
				if ( isset( $item['schedule']['last_run'] ) ) {
					...
					if($last_run_local)
						return gmdate('M j Y', $last_run_local) . ' at ' . gmdate('G:i', $last_run_local);
					else
