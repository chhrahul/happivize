	...
	public static function filter_price_html( $price, $product ) {
		...
		if ( ! empty( $product_level_schemes ) ) {
			...
			if ( $force_subscription === 'yes' ) {
				...
				if ( $price_overrides_exist ) {
					$suffix = ' <small class="wcsatt-sub-options">' . sprintf( _n( 'or %s', '&ndash; subscription plans available %s', count( $product_level_schemes ), WCS_ATT::TEXT_DOMAIN ), $suffix_price_html ) . '</small>';
				} else {
