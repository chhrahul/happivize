...
class WCS_Email_New_Renewal_Order extends WC_Email_New_Order {
	...
	function trigger( $order_id, $order = null ) {
		...
		if ( $order_id ) {
			...
			$orderss = wc_get_order( $order_id );

			$subjectt = "";
			$prdtc_qty_sum_total = "";
			$itemsss = $orderss->get_items();
			foreach ( $itemsss as $item_id => $item_data ) {

			  $prdct_name = $item_data['name'];
			  $prdtc_qty = $orderss->get_item_meta($item_id, '_line_total', true);
			  $prdtc_qty_sum_total = $prdtc_qty_sum_total + $prdtc_qty;
			  $subjectt .= " $prdct_name,";
			  
			}
			$subjectt_pro = rtrim($subjectt,',');

			$this->subject  =  "Payment made - ".$subjectt_pro." for $".$prdtc_qty_sum_total;
