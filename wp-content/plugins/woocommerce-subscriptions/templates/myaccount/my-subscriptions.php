...
<div class="woocommerce_account_subscriptions">
	...
	$getstatus = esc_attr( wcs_get_subscription_status_name( $subscription->get_status() ));
		if($getstatus == "Active"){
			$active_status = count($getstatus);
		}
		else{
			$other_status = count($getstatus);
		}
	}
	...
	<h2><?php esc_html_e( 'My Payment Plans', 'woocommerce-subscriptions' ); ?></h2>
	<?php } ?>
	...
	?>
		...
		<tr class="order">
			...
			<td class="subscription-actions order-actions">
				<?php $actions = WC_Subscriptions_Change_Payment_Gateway::change_payment_method_button( array(), $subscription ); ?>
				<?php if ( ! empty( $actions ) ) : ?>
				  <?php foreach ( $actions as $key => $action ) : ?>
				    <a style="margin-top: 2.5px !important;" href="<?php echo esc_url( $action['url'] ); ?>" class="button <?php echo sanitize_html_class( $key ) ?>"><?php echo esc_html( $action['name'] ); ?></a>
				  <?php endforeach; ?>
				<?php endif; ?>
				<a style="margin-top: 2.5px !important;" href="<?php echo esc_url( $subscription->get_view_order_url() ) ?>" class="button view"><?php echo esc_html_x( 'View', 'view a subscription', 'woocommerce-subscriptions' ); ?></a>
