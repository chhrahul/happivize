...
class WC_Form_Handler {
    ...
    if (strpos($url,'offers') !== false) {
 //echo 'yes';   die;
 $cart_url = '';
 wp_redirect( 'https://happivize.com/cart' );
exit;
	...
	private static function add_to_cart_handler_grouped( $product_id ) {
		...
		if ( ! empty( $_REQUEST['quantity'] ) && is_array( $_REQUEST['quantity'] ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				wc_add_notice( __( 'Please choose an items you wish to add to your cart&hellip;', 'woocommerce' ), 'error' );
