...
class WC_Email_Customer_Refunded_Order extends WC_Email {
	...
	public function trigger( $order_id, $partial_refund = false, $refund_id = null ) {
		...
		/* Send Refunded Email to Admin Also */
		$this->send( $this->get_option( 'recipient', get_option( 'admin_email' ) ), $this->get_subject(), $this->get_content(), $this->get_headers(), $this->get_attachments() );
		/* Send Refunded Email to Admin Also */
	}
