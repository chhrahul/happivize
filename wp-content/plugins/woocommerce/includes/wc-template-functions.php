...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
        ...
        foreach ( $countries as $ckey => $cvalue ) {
            if($ckey == 'GB' || $ckey == 'US' || $ckey == 'AU' || $ckey == 'CA' || $ckey == 'CA')
            {
                continue;
            }
            $count[$ckey] = $cvalue;
        ...
        $newarray = array('US'=>'United States (US)','AU'=>'Australia','CA'=>'Canada','NZ'=>'New Zealand','GB'=>'United Kingdom (UK)','---'=>'---');
        
        $merge_countries = array_merge($newarray,$count);
        if ( 1 === sizeof( $countries ) ) {
					...
					foreach ( $merge_countries as $ckey => $cvalue ) {
					$dis = '';
          if($ckey == '---')
          {
              $dis = 'disabled=disabled';
          }
          	$field .= '<option '.$dis.' value="' . esc_attr( $ckey ) . '" '. selected( $value, $ckey, false ) . '>'. __( $cvalue, 'woocommerce' ) .'</option>';
					}
