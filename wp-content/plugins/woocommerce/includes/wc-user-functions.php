...
function wc_create_new_customer( $email, $username = '', $password = '' ) {
	...
	if ( email_exists( $email ) ) {
		return new WP_Error( 'registration-error-email-exists', __( 'An account is already registered with your email address. < a href="https://happivize.com/login/?a_redirect_to=https://happivize.com/checkout/">Please login</a>.', 'woocommerce' ) );
	}
	...
	if ( $results ) {
		...
		foreach ( $results as $result ) {
			...
			$downloads[] = array(
				...
				'file'                => $download_file,
				'parent_pro'          => $order->post->post_parent
			);
